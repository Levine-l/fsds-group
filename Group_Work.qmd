...
When combined with availability patterns from Section 1.1, 22.91% of listings are both available for more than 90 days and priced above £200, suggesting operation as full-time, high-priced accommodation. This aligns with findings by @barron_effect_2021 that Airbnb expansion encourages the conversion of residential housing into short-term commercial assets.
...
The results show that Airbnb activity is highly concentrated in central London, rather than evenly distributed across the city. This spatial concentration reinforces earlier findings that Airbnb activity in London exhibits characteristics of being “out of control,” combining regulatory exceedance with intensive use in high-value areas. 
...
To further assess whether Airbnb activity in London is “out of control,” we calculate the proportion for professional landlord.
...
The results show a clear alignment between spatial concentration and professionalisation. Boroughs with the highest number of listings also exhibit the highest multi-lister shares. Westminster records the largest number of listings (8,347) and the highest multi-lister share (84%).
```{python}
...
These figures indicate that the majority of listings in London's core areas are not managed by home hosts but rather by professional operators operating multiple units, which indicates a large-scale short-term rental business model.

**Conclusion**

Overall, Airbnb activity is highly concentrated in central and inner-west London, while suburban boroughs exhibit much lower densities (@shabrina_airbnb_2022). This suggests that the Airbnb-related housing pressure is not city-wide, but concentrated in a limited number of strategic hotspots. At the borough level, Airbnb in London therefore exhibits characteristics of being “out of control” in a spatially concentrated and professionalised manner, with disproportionate impacts on long-term housing availability in central areas.
...
**Assumption**
 ...
 We assume that extreme clustering at the MSOA level indicates intensive, potentially commercial short-term rental activity rather than dispersed home-sharing. Listing counts were calculated by spatially joining Airbnb listings to Westminster MSOAs following geometric clipping.
...
The results show that Airbnb activity within Westminster is highly uneven. While the borough contains over 8,300 listings, most MSOAs host relatively few, whereas a small number record several hundred to over 1,000 listings each.

**Conclusion**

This pattern indicates that Airbnb activity in Westminster is intensely localised within a small number of MSOAs， which are more like informal hotel districts than home-sharing areas, reinforcing evidence that Airbnb activity in London is “out of control” in specific high-pressure locations. 


### 1.6 MSOA-level CLustering

To identify distinct neighbourhood-level patterns of Airbnb activity in London, we classify MSOAs using clustering based on three indicators: listing density, average price, and the share of listings exceeding the 90-day threshold.

**Assumption**

We assume that these three variables capture the core dimensions of Airbnb intensity and commercialisation at the neighbourhood scale. A three-cluster k-means solution is used to balance interpretability with sufficient differentiation across MSOAs.
...
The clustering reveals three distinct Airbnb typologies. The high-intensity cluster is concentrated in central London and combines high listing densities, higher prices, and elevated rates of 90-day exceedance, indicating areas where Airbnb operates most clearly as a large-scale commercial activity. A second, medium-intensity cluster shows lower listing densities and prices but the highest share of listings exceeding 90 days, suggesting more “embedded” commercial use within residential neighbourhoods. This pattern aligns with findings by @barron_effect_2021 that housing impacts arise when dwellings are persistently shifted from long-term to short-term rental use, even outside prime tourist zones. The low-intensity cluster, covering much of suburban London, exhibits fewer listings, moderate prices, and lower exceedance rates, closely reflecting casual home-sharing.

**Conclusion**

This MSOA-level typology confirms that Airbnb operates differently across neighbourhoods. High- and medium-intensity clusters identify where Airbnb is most likely to function as a sustained business activity, suggesting that area-specific regulatory responses are more appropriate than uniform city-wide measures.
...
### 1.8 Limitation
This analysis has several limitations. Availability_365 may overstate non-compliance, high prices may reflect quality rather than professionalisation, spatial aggregation may mask local variation, and the multi-listing threshold likely underestimates commercial activity. The findings should therefore be interpreted as indicative patterns rather than precise measures of illegality.
...
affected_gdf = gpd.GeoDataFrame(
    affected_properties,
    geometry=gpd.points_from_xy(affected_properties.longitude, affected_properties.latitude),
    crs="EPSG:4326"
).to_crs(boroughs.crs)
...
#Borough boundaries for context
boroughs.boundary.plot(ax=ax, linewidth=0.8, color="black", alpha=0.7)

#All listings with grey
gdf_listings.plot(
    ax=ax,
    markersize=0.6,
    color="grey",
...
#Affected listings with red
affected_gdf.plot(
    ax=ax,
    markersize=2.0,
    color="red",
    alpha=0.1,
    label="Affected listings"
)

ax.set_title("All Airbnb Listings & Listings Affected by the Proposal", fontsize=14, pad=12)
ax.set_axis_off()
ax.legend(loc="lower left", frameon=True)

plt.show()
```
Here is a simple graph shows all lisings and those be affected. The grey points show the full distribution of Airbnb listings across London, while the red points highlight those affected by the proposal.
...
For the Mayor, a regulatory focus on professional short-term rental operators offers strong political and strategic credibility. Targeting commercial activity reflects the empirical patterns identified in this study and allows the policy to be framed as evidence-led, proportionate, and fair. By distinguishing casual home-sharing from business-oriented short-term rentals, housing pressures can be addressed without penalising ordinary residents, consistent with urban planning best practice (@gurran_when_2017). This approach also supports targeted fiscal measures, such as higher Council Tax rates or levies on commercial rentals, which can be justified as internalising external costs in high-impact areas. However, professional landlords are likely to resist regulation through lobbying or legal challenges (@barron_effect_2021), and the Mayor must clearly communicate the policy as an independent, data-driven strategy rather than a reactive political response.

### 4.2 Implication for Residents 

For residents, particularly in central boroughs, regulation may improve housing availability and neighbourhood stability. As shown in Question 3, many professional Airbnb listings operate on a near full-time basis, reducing long-term rental supply. Regulating this segment may encourage some properties to return to the long-term market, easing pressure in the most affected areas, even if city-wide rent reductions remain limited (@garcia-lopez_short-term_2020). Reduced clustering may also improve quality of life by lowering noise, turnover, and social disruption (@gurran_when_2017). Nevertheless, residents may face trade-offs: Airbnb provides flexible accommodation for students, temporary workers, and households in transition, and tighter regulation may reduce these options in central locations. Cost shifting toward hotels or serviced apartments may also disproportionately affect middle-income users.

### 4.3 Implication for City

At the city scale, targeted regulation may improve housing market efficiency by limiting the conversion of residential properties into commercial short-term rentals in high-pressure areas. Enhanced registration and monitoring can strengthen governance capacity and support more adaptive policy responses. However, displacement effects remain a risk, as activity may shift to other boroughs or platforms rather than disappear (@garcia-lopez_short-term_2020). Given the uneven spatial distribution of Airbnb impacts, uniform city-wide regulation may be inefficient, highlighting the need for spatially differentiated enforcement.


## Question 5: Can the story be reframed as a positive one about social mobility or housing opportunity?

The controversy surrounding multi-property Airbnb ownership risks oversimplifying London’s short-term rental market. Evidence from Questions 1–3 shows that the core problem is not casual home-sharing, but the dominance of professional landlords who control a disproportionate share of listings and operate intensively in central boroughs. This distinction creates scope for a more constructive reframing of the debate.

Rather than portraying Airbnb as inherently harmful, the Mayor can focus on how benefits are distributed and who gains from short-term rental activity. For some London residents, Airbnb still provides supplementary income and economic resilience, allowing households to offset rising living costs or maintain housing security (@arenoe_game_2015; @sundararajan_sharing_2016). In this limited sense, genuine home-sharing can support household stability and modest forms of social mobility.

However, this positive narrative cannot be generalised. As shown in Questions 1–3, professional operators dominate high-demand central areas, transforming Airbnb from a sharing platform into a mechanism for housing commodification. A credible reframing therefore requires differentiation rather than blanket defence.

The Mayor can emphasise that regulation aims to protect socially beneficial home-sharing while constraining large-scale commercial use. Hosts with one or two listings remain part of the sharing economy, whereas professional operators face stricter controls. Regulation can thus be framed not as anti-innovation, but as safeguarding ordinary residents’ income opportunities from being crowded out by property accumulation.

In this way, the controversy can be repositioned as part of a broader commitment to balancing innovation, social mobility, and housing access, grounded in empirical evidence from the London context.
